...
public class DuplicateConditionAnnotator implements Annotator {
    ...
    public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
        ...
        if (!(element instanceof Perl6IfStatement))
            return;
        Perl6IfStatement.IfStructure structure = ((Perl6IfStatement)element).getStructure();
        Perl6IfStatement.IfPart[] parts = structure.getConditionalParts();
        if (parts.length < 2)
            return;

        // Compute a normalization of all the conditions group them on that.
        Map<String, List<Perl6PsiElement>> duplicated = new HashMap<>();
        for (Perl6IfStatement.IfPart part : parts) {
            Perl6PsiElement condition = part.getCondition();
            if (condition == null)
                continue;
            duplicated.computeIfAbsent(normalize(part.isWith(), condition), k -> new ArrayList<>()).add(condition);
        }

        // If there are any identical conditions, report them.
        if (duplicated.size() == parts.length) // Fail fast, all distinct
            return;
        for (Map.Entry<String, List<Perl6PsiElement>> maybeIdentical : duplicated.entrySet()) {
            List<Perl6PsiElement> identical = maybeIdentical.getValue();
            if (identical.size() > 1)
    ...
    private static String normalize(boolean with, Perl6PsiElement condition) {
        return (with ? "with\0" : "if\0") + normalize(condition);
    }

    private static String normalize(Perl6PsiElement condition) {
        ...
        if (condition instanceof Perl6InfixApplication) {
            ...
            if (op != null && operands.length == 2 &&
                ...
                String lhsNorm = normalize((Perl6PsiElement)operands[0]);
                String rhsNorm = normalize((Perl6PsiElement)operands[1]);
        ...
        while ((current = Perl6PsiUtil.skipSpaces(current, true, true)) != null) {
            parts.add(current instanceof Perl6PsiElement
                      ? normalize((Perl6PsiElement)current)
                      : current.getText());
