...
public class JdkComboBox extends SdkComboBoxBase<JdkComboBox.JdkComboBoxItem> {
  ...
  public void setEditButton(@NotNull JButton editButton,
    ...
    editButton.addActionListener(e -> {
      ...
      if (projectJdk != null) {
        Perl6ProjectStructureConfigurable.getInstance(project).select(projectJdk, true);
      }
    });
    addActionListener(e -> {
      final JdkComboBoxItem selectedItem = getSelectedItem();
      if (selectedItem instanceof ProjectJdkComboBoxItem) {
        editButton.setEnabled(Perl6ProjectStructureConfigurable.getInstance(project).getProjectSdksModel().getProjectSdk() != null);
      }
      else {
